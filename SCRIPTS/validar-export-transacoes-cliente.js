...
/**
 * CLI-ponte FINO (V1 - extrato individual por cliente), espelhando
 * exatamente o contrato de SCRIPTS/validar-export-vendas.js. NUNCA
 * reimplementa nenhuma regra de validacao - so le o arquivo do disco,
 * chama `lerExportTransacoesCliente(buffer, opcoes)` sem alteracao, e
 * imprime um contrato JSON minimo em stdout + exit code deterministico.
 *
 * Uso:
 *   node SCRIPTS/validar-export-transacoes-cliente.js <caminho-absoluto-do-xls>
 *
 * Contrato de saida (stdout - SEMPRE uma unica linha JSON, nunca payload
 * de negocio/PII):
 *   sucesso:        {"ok":true,"rows":17}
 ...
 * linhas de transacoes/PII.
 *
 * Este CLI e' SOMENTE VALIDACAO - nunca move, copia, apaga ou renomeia o
 * arquivo, nunca decide EXPORT_STAGE/EXPORTADOS (isso e' governanca
 * exclusiva do Agent C#), nunca publica.
 */

const fs = require('fs');
const path = require('path');
const { lerExportTransacoesCliente, ErroLeituraExportTransacoes } = require(path.join(__dirname, '..', 'SERVICO', 'leitor-export-transacoes-cliente'));
...
function main() {
  ...
  if (!caminho) {
    process.stderr.write('Uso: node validar-export-transacoes-cliente.js <caminho-absoluto-do-xls>\n');
  ...
  try {
    const resultado = lerExportTransacoesCliente(buffer, { nomeArquivo: path.basename(caminho) });
    emitirContrato({ ok: true, rows: resultado.linhas.length });
    process.exitCode = EXIT_OK;
  } catch (e) {
    if (e instanceof ErroLeituraExportTransacoes) {
    ...
    // como erro de infraestrutura, nunca como "ok:false" do Reader.
